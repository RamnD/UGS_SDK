...
internal static class AnalyticsCustomEventEnricher
{
    ...
    public const string UnityPlayerIdParam = "unity_player_id";

    public static void ApplyUnityPlayerId(CustomEvent customEvent, string playerId)
    {
        if (customEvent == null || string.IsNullOrEmpty(playerId) || playerId == "unknown")
            return;

        customEvent.Add(UnityPlayerIdParam, playerId);
    }
}
